...
import { getDonationLog, addDocument, deleteDocument } from "@/lib/firestore";
...
export default function AdminDonationsLogPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const data = await getDonationLog();
  ...
  async function handleAddLog(e) {
    ...
    try {
      const res = await addDocument("donationLog", newLog);
      setLogs([{ id: res.id, ...newLog }, ...logs]);
      setDonorName("");
      setAmount("");
      setNote("");
      setMessage("✅ Donation entry recorded privately!");
    } catch {
      setLogs([{ id: Date.now().toString(), ...newLog }, ...logs]);
      setDonorName("");
      setAmount("");
      setNote("");
      setMessage("✅ Donation entry recorded locally!");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleDelete(id) {
    ...
    try {
      await deleteDocument("donationLog", id);
    } catch {}
      ...
      <div className="mb-6 flex justify-between items-end">
        <div>
          <h1 className="font-[var(--font-heading)] text-xl text-white">
            Private Donation Activity Log
          </h1>
          <p className="text-xs text-muted mt-1">
            Manually log received UPI donations for committee bookkeeping. This is strictly private and never shown to the public.
          </p>
        </div>
                ...
                <tbody className="divide-y divide-[rgba(255,255,255,0.06)] text-[#d4c5b0]">
                  {logs.map((log) => (
                    <tr key={log.id} className="hover:bg-white/[0.02]">
                      <td className="py-3 font-mono text-[0.7rem] text-muted">
                        {log.loggedAt ? new Date(log.loggedAt).toLocaleDateString("en-IN") : "Recent"}
                      </td>
